...
let swiftGenPrefix: String = "Strings"
var stringsCount: Int = 0

getAllUsedStrings()

func getAllUsedStrings() {
    let filesPath: [String] = [String](CommandLine.arguments.suffix(from: 1)).filter { !$0.hasSuffix("Strings.swift") }
    var stringsNames: [String] = []
    Progress(filesPath, configuration: [ProgressString(string: "|"),
                                        ProgressStringBlock { index, count in "\((index < count ? "\(index)/\(count)" : "✅ \(stringsCount) strings are used")) (\(min(index + 1, count))/\(count))" },
                                        ProgressBarLine(barLength: 48),
                                        ProgressPercent(),
                                        ProgressString(string: "|")]).forEach {
        let url: URL = URL(fileURLWithPath: $0)
        guard let data = try? Data(contentsOf: url) else { return }
        guard let code = String(data: data, encoding: .utf8) else { return }
        guard let regex = try? NSRegularExpression(pattern: #"Strings\.(?<stringName>.*?)(?:[^A-Za-z0-9\.\-\_])"#) else { return }
        let matches: [NSTextCheckingResult] = regex.matches(in: code, options: [], range: NSRange(location: 0, length: code.count))
        stringsCount += matches.count
        stringsNames.append(contentsOf: matches.compactMap {
            let stringNameRange: NSRange = $0.range(withName: "stringName")
            guard let substringRange = Range(stringNameRange, in: code) else { return nil }
            return String(code[substringRange])
        })
    }
    let formattedStrings: String = stringsNames.sorted(by: <).joined(separator: "\n").lowercased()
    let directoryUrl: URL = URL(fileURLWithPath: CommandLine.arguments[0]).deletingLastPathComponent()
    try? formattedStrings.write(to: directoryUrl.appendingPathComponent("result"), atomically: true, encoding: .utf8)
}
...
public struct ProgressIndex: ProgressElementType {
    ...
    public func value(_ progressBar: ProgressBar) -> String {
        return "\(progressBar.index)/\(progressBar.count)"
    }
}
...
public struct ProgressStringBlock: ProgressElementType {
    let block: (_ index: Int, _ count: Int) -> String

    public init(block: @escaping (_ index: Int, _ count: Int) -> String) {
        self.block = block
    }

    public func value(_ bar: ProgressBar) -> String {
        block(bar.index, bar.count)
    }
}
